...
export async function GET(request: Request) {
  ...
  if (session.authDisabled || !session.userId) return NextResponse.json({ error: "Authentication is required to read a QuickBooks sync." }, { status: 401 });
  ...
  const { data, error } = await supabase
    ...
    .eq("id", syncId)
    .single();
  if (error || !data) return NextResponse.json({ error: error?.message || "QuickBooks sync not found." }, { status: 404 });
...
export async function POST(request: Request) {
  const session = await requireApiSession();
  if (!session.ok) return session.response;
  if (session.authDisabled || !session.userId) return NextResponse.json({ error: "Authentication is required to sync QuickBooks." }, { status: 401 });
  ...
  const supabase = await createClient();
  const connection = await selectedQuickBooksConnection(supabase, tenantId, companyId);
  ...
  if (active) return NextResponse.json({ error: "A QuickBooks sync is already in progress for this connection. Please wait for it to finish.", syncId: active.id, status: active.status }, { status: 409 });
  const { error: queueError } = await supabase.from("accounting_sync_runs").insert({ id: syncId, tenant_id: tenantId, company_id: companyId, integration_id: connection.id, provider: "quickbooks", sync_type: "finance_and_vat", status: "queued" });
  if (queueError) {
    ...
    return NextResponse.json({ error: duplicate ? "A QuickBooks sync is already in progress for this connection. Please wait for it to finish." : queueError.message }, { status: duplicate ? 409 : 500 });
  }

  after(() => runQuickBooksSync({ supabase, connection, syncId, sessionUserId: session.userId!, body, tenantId, companyId, asOfDate }));
  return NextResponse.json({ queued: true, syncId, status: "queued" }, { status: 202 });
}

async function runQuickBooksSync({ supabase, connection, syncId, sessionUserId, body, tenantId, companyId, asOfDate }: {
  supabase: Awaited<ReturnType<typeof createClient>>;
  connection: Awaited<ReturnType<typeof selectedQuickBooksConnection>>;
  syncId: string;
  ...
  try {
    ...
    const auth = await authenticatedQuickBooks(supabase, connection);
    ...
    const sync = await fetchQuickBooksSyncData(auth, asOfDate, vatPeriod);
    ...
    if (imported === 0 && sync.warnings.length) throw new Error(`QuickBooks returned no data — ${sync.warnings.join("; ")}`);
    if (sync.warnings.length) reportError(new Error(`QuickBooks sync partial: ${sync.warnings.join("; ")}`), { route: "quickbooks/sync", syncId, tenantId, companyId });
