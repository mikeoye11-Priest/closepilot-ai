...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase.from("accounting_integrations").select("id,tenant_id").eq("id", integrationId).eq("provider", "sage").eq("user_id", session.userId).single();
