...
// The human phrase for where the figures came from, used in the accounts packs so
// a QuickBooks/Sage/uploaded set is never described as "the Xero ledger".
export function ledgerPhrase(provider?: string): string {
  switch (provider) {
    case "quickbooks": return "the QuickBooks ledger";
    case "sage": return "the Sage ledger";
    case "upload": return "the uploaded accounting records";
    case "demo": return "the demo dataset";
    case "xero": return "the Xero ledger";
    default: return "the connected ledger";
  }
}
...
function buildNotes(pl: PL, bs: BS, debtors: Aged, creditors: Aged, cashBalance: number, sourceLabel: string): Note[] {
  ...
  const notes: Note[] = [
    { title: "Basis of preparation", body: `These management accounts have been prepared from ${sourceLabel} for internal management purposes. They are unaudited and do not constitute statutory financial statements.` },
  ];
...
export function buildManagementAccounts(statements: SyncStatements, findings: ManagementAccountsFinding[] = []) {
  ...
  return {
    meta: { companyName: statements.companyName ?? "Company", asOfDate: statements.asOfDate, periodStart: statements.periodStart ?? `${statements.asOfDate.slice(0, 4)}-01-01`, currency: statements.currency ?? "GBP", sourceLabel: ledgerPhrase(statements.sourceProvider) },
    ...
    notes: buildNotes(pl, bs, debtors, creditors, cashBalance, ledgerPhrase(statements.sourceProvider)),
  };
}
...
`;
  const title = `${esc(meta.companyName)} — Management Accounts`;
  const inner = `
  <header>
  ...
  <footer>
    Generated by ClosePilot from ${meta.sourceLabel}. These management accounts are prepared for internal management purposes and are subject to review by the preparer. Any AI-drafted narrative is grounded in the figures above and must be reviewed before issue. Figures are rounded to the nearest £.
  </footer>`;
