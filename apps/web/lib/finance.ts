...
export function assistantAnswer(question: string, score: number, findings: Finding[], forecast: CashForecastPoint[]) {
  ...
  if (normalized.includes("score")) {
    return `Review Quality is ${score}/100. The main issue affecting the score is ${top?.title ?? "open finance risks"}. Completing high-priority recommendations should improve the score fastest.`;
  }
