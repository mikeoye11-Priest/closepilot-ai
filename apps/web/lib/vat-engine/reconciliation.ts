...
export function reconcileVatReturn(vatReturn: VatReturn, vatControlBalance?: number, contributions: VatBoxContribution[] = [], transactions: VatTransaction[] = [], hmrcPayment?: number): VatReconciliationResult[] {
...
function boxCheck(name: string, expected: number, actual: number, tolerance = 1): VatReconciliationResult {
  ...
  return {
    ...
    detail: passed
      ...
      : `FAIL: expected ${fc(expected)} vs actual ${fc(actual)}. Difference ${fc(difference)}.`,
  };
}
